// LinkedIn Learning-specific content script
// Handles LinkedIn Learning platform integration for video detection, transcript parsing, and navigation

class LinkedInLearningIntegration {
  constructor() {
    this.platform = 'linkedin-learning';
  ...
  init() {
    console.log('LinkedIn Learning Integration: Initializing...');
  ...
  setup() {
    // Enhance video detection for LinkedIn Learning
  ...
  enhanceVideoDetection() {
    // LinkedIn Learning-specific video selectors
    const linkedinVideoSelectors = [
      'video[data-control-name="course_video"]',
      '.video-player video',
      '.lls-player-container video',
      '.vjs-tech',
      '[data-component-name="video-player"] video',
      '.ember-view .video-player video'
    ];
    ...
    if (originalFindVideo) {
      window.LearningAssistant.prototype.findVideo = async function() {
        ...
        for (const selector of linkedinVideoSelectors) {
  ...
  setupTranscriptParsing() {
    // Look for LinkedIn Learning transcript containers
    const transcriptSelectors = [
      '.lls-transcript-container',
      '.transcript-container',
      '[data-control-name="transcript"]',
      '.lls-course-transcript',
      '.video-transcript',
      '[class*="transcript"]'
    ];
  ...
  enhanceTranscript(container) {
    console.log('Enhancing LinkedIn Learning transcript:', container);

    // Add click-to-seek functionality
    container.addEventListener('click', (e) => {
      const transcriptLine = e.target.closest('[data-time], .transcript-line, .lls-transcript-line');
      if (transcriptLine) {
  ...
  extractTimestamp(element) {
    // Try different timestamp formats used by LinkedIn Learning
    ...
    // Try to parse from text content (format: mm:ss or [mm:ss])
    ...
    const timeElement = element.querySelector('[data-time], .timestamp, .time');
    if (timeElement) {
  ...
  syncTranscriptHighlight(currentTime) {
    ...
    const transcriptLines = this.transcriptContainer.querySelectorAll(
      '[data-time], .transcript-line, .lls-transcript-line'
    );
  ...
  setupNavigation() {
    // Find next lecture button patterns in LinkedIn Learning
    this.nextLectureSelectors = [
      ...
      'button[aria-label*="Next"]',
      '.next-video-button',
      'a[href*="learning"][href*="next"]'
    ];
  }

  navigateToNext() {
    console.log('LinkedIn Learning: Attempting to navigate to next lecture');
  ...
  navigateViaMenu() {
    ...
    const menuSelectors = [
      '.lls-course-navigation',
      '.course-outline',
      '.video-list',
      '.chapter-list',
      '[data-control-name="course_outline"]'
    ];
    ...
    for (const selector of menuSelectors) {
      ...
      if (menu) {
        ...
        const currentLecture = menu.querySelector('.active, [aria-current="page"], .current');
        if (currentLecture) {
  ...
  findNextLectureInMenu(currentElement) {
    ...
    // Try parent's next sibling (different chapter/section)
    const parent = currentElement.closest('.chapter, .section, .lls-chapter');
    if (parent) {
  ...
  isLectureLink(element) {
    ...
    return (
      ...
      ariaLabel.includes('chapter') ||
      controlName.includes('video') ||
  ...
  showNavigationGuidance() {
    ...
    guidance.innerHTML = `
      ...
      <div style="font-weight: 600; margin-bottom: 8px;">Great progress!</div>
      <div style="color: #ccc; line-height: 1.4;">
        You've completed this video. Navigate to the next one using the course menu or player controls.
      </div>
...
// Initialize LinkedIn Learning integration
const linkedinLearningIntegration = new LinkedInLearningIntegration();